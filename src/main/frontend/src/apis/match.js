...
export const fetchMatchAnalysis = async (matchId) => {
    const {
        data
    } = await axios.get(`/result/match/${matchId}`);
    return data;
}
...
export const fetchPendingMatchList = async () => {
    const {
        data
    } = await axios.get(`/match/waitlist`);
    return data;
}
...
export const postWriteMatch = async (props) => {
    const { data } = await axios.post(`/result/match/${props.matchId}?teamName=${props.teamName}`, props.postBody);
    return data;
}

// 팀 예정 경기 목록 조회
export const fetchPlanMatchList = async (teamName) => {
    const {data} = await axios.get(`/team/${teamName}/match?matchState=expected`);
    return data;
}
...
export const fetchFinishedMatchList = async (teamName) => {
    const {
        data
    } = await axios.get(`/team/${teamName}/match?matchState=finish`);
    return data;
}
...
export const fetchCreatedMatchList = async (teamName) => {
    const {
        data
    } = await axios.get(`/match/${teamName}/mylist`);
    return data;
}